...
public class AuthFactory : IAuthFactory
{
    ...
    public async Task<(bool Success, LoginResponse? Response, string? Error)> LoginAsync(LoginRequest request)
    {
        ...
        // Auth:RequireEmailVerification:User=true ise mail doğrulaması zorunlu. Default false: mevcut/yeni kullanıcılar mail beklemeden login olur.
        var requireEmailVerify = _config.GetValue<bool>("Auth:RequireEmailVerification:User", false);
        if (requireEmailVerify && !user.IsEmailVerified && !string.IsNullOrWhiteSpace(user.Email))
