...
public class CrmQualityFactory : ICrmQualityFactory
{
    private readonly ICrmQualityChecklistEntityService _checklistEs;
    private readonly ICrmQualityQuestionEntityService _questionEs;
    private readonly ICrmQualitySubCriteriaEntityService _subCriteriaEs;
    private readonly ICrmQualityEvaluationEntityService _evaluationEs;
    private readonly ICrmQualityAnswerEntityService _answerEs;
    private readonly ICrmQualityAnswerSubCriteriaSelectionEntityService _selectionEs;
    private readonly ICrmQualityScoreThresholdEntityService _thresholdEs;
    ...
    public CrmQualityFactory(
        ICrmQualityChecklistEntityService checklistEs,
        ICrmQualityQuestionEntityService questionEs,
        ICrmQualitySubCriteriaEntityService subCriteriaEs,
        ICrmQualityEvaluationEntityService evaluationEs,
        ICrmQualityAnswerEntityService answerEs,
        ICrmQualityAnswerSubCriteriaSelectionEntityService selectionEs,
        ICrmQualityScoreThresholdEntityService thresholdEs,
    ...
    public async Task<List<CrmQualityChecklistDto>> GetChecklistsAsync(int? customerId)
    {
    ...
    public async Task<CrmQualityChecklistDetailDto?> GetChecklistByUidAsync(Guid uid, int? customerId)
    {
        var query = _checklistEs.GetAllQueryable()
        ...
        return new CrmQualityChecklistDetailDto
        {
            Id = checklist.Id,
            Uid = checklist.Uid,
            Name = checklist.Name,
            ...
            ScoringMethodName = CrmQualityScoringMethods.GetById(checklist.ScoringMethodId)?.Description ?? "",
    ...
    public async Task<CrmQualityChecklistDto> CreateChecklistAsync(CreateChecklistRequest req, int? customerId)
    {
        var checklist = new CrmQualityChecklist
        {
    ...
    public async Task<(bool Success, string? Error, CrmQualityQuestionDto? Question)> AddQuestionAsync(
        Guid checklistUid, CreateQuestionRequest req, int? customerId)
    {
        ...
        var question = new CrmQualityQuestion
        {
        ...
        {
            var subCriteria = req.SubCriteria.Select((sc, i) => new CrmQualityQuestionSubCriteria
            {
    ...
    public async Task<List<CrmQualityEvaluationDto>> GetEvaluationsAsync(
    ...
    public async Task<CrmQualityEvaluationDetailDto?> GetEvaluationByUidAsync(Guid uid, int? customerId)
    {
        var query = _evaluationEs.GetAllQueryable()
            .Include(e => e.Checklist)
            .Include(e => e.CallRecord)
        ...
        var dto = new CrmQualityEvaluationDetailDto
        {
            Id = evaluation.Id,
            Uid = evaluation.Uid,
            ...
            StatusName = CrmQualityEvaluationStatuses.GetById(evaluation.StatusId)?.Description ?? "",
            ...
            Answers = evaluation.Answers
                ...
                .Select(a => new CrmQualityAnswerDto
                {
                    Id = a.Id,
                    QuestionId = a.QuestionId,
                    QuestionText = a.Question?.Text ?? "",
                    GroupName = a.Question?.GroupName,
                    ScoringTypeId = a.Question?.ScoringTypeId ?? CrmQualityScoringTypes.Ids.Scored,
                    ...
                    AppliedPenaltyTypeName = CrmQualityPenaltyTypes.GetById(a.AppliedPenaltyTypeId)?.Description,
    ...
    public async Task<(bool Success, string? Error, CrmQualityEvaluationDto? Evaluation)> CreateEvaluationAsync(
        CreateEvaluationRequest req, int evaluatorPersonnelId, int? customerId)
    {
        ...
        var evaluation = new CrmQualityEvaluation
        {
            ...
            StatusId = CrmQualityEvaluationStatuses.Ids.Draft,
        ...
        Guid uid, SubmitEvaluationRequest req, int? customerId)
    {
        ...
        evaluation!.StatusId = CrmQualityEvaluationStatuses.Ids.Draft;
        ...
        Guid uid, SubmitEvaluationRequest req, int? customerId)
    {
        ...
        foreach (var answer in answers)
        {
            ...
            // Puan hesaplama
            if (!answer.IsNotApplicable && question.ScoringTypeId == CrmQualityScoringTypes.Ids.Scored)
            {
                if (scoringMethod == CrmQualityScoringMethods.Ids.Maximum)
                {
                ...
                else if (scoringMethod == CrmQualityScoringMethods.Ids.CriteriaTotal)
                {
            ...
            {
                if (answer.AppliedPenaltyTypeId == CrmQualityPenaltyTypes.Ids.YellowCard)
                    yellowCards++;
                else if (answer.AppliedPenaltyTypeId == CrmQualityPenaltyTypes.Ids.RedCard)
                    redCards++;
            }
        }
        ...
        evaluation.StatusId = CrmQualityEvaluationStatuses.Ids.Completed;
    ...
    /// </summary>
    private async Task<(bool Success, string? Error, CrmQualityEvaluation? Evaluation)> LoadEvaluationForEdit(
        Guid uid, int? customerId)
    {
        ...
        if (evaluation.StatusId == CrmQualityEvaluationStatuses.Ids.Completed)
            return (false, "Bu degerlendirme zaten tamamlanmis", null);
        if (evaluation.StatusId == CrmQualityEvaluationStatuses.Ids.Cancelled)
    ...
    /// </summary>
    private async Task<List<CrmQualityAnswer>> ReplaceAnswersAsync(
        CrmQualityEvaluation evaluation, SubmitEvaluationRequest req)
    {
        ...
        var answers = new List<CrmQualityAnswer>();
        foreach (var answerReq in req.Answers)
        {
            var answer = new CrmQualityAnswer
            {
        ...
        var selections = new List<CrmQualityAnswerSubCriteriaSelection>();
        foreach (var answerReq in req.Answers)
        {
            ...
            {
                selections.Add(new CrmQualityAnswerSubCriteriaSelection
                {
                    AnswerId = answer.Id,
                    SubCriteriaId = scId
                });
    ...
    public async Task<(bool Success, string? Error)> CancelEvaluationAsync(Guid uid, int? customerId)
    {
        ...
        evaluation.StatusId = CrmQualityEvaluationStatuses.Ids.Cancelled;
        evaluation.UpdatedAt = DateTime.UtcNow;
    ...
    public async Task<List<CrmQualityScoreThresholdDto>> GetThresholdsAsync(int? customerId)
    {
        ...
        return thresholds.Select(t => new CrmQualityScoreThresholdDto
        {
            Id = t.Id,
    ...
    public async Task<(bool Success, string? Error)> UpsertThresholdAsync(UpsertScoreThresholdRequest req, int? customerId)
    {
        ...
        {
            var threshold = new CrmQualityScoreThreshold
            {
    ...
    public async Task<CrmQualityDashboardDto> GetDashboardAsync(int? customerId, DateTime? from = null, DateTime? to = null)
    {
        var query = _evaluationEs.GetAllQueryable()
            .Include(e => e.EvaluatedPersonnel!).ThenInclude(p => p!.User)
            .AsQueryable();
        ...
        var completed = evaluations.Where(e => e.StatusId == CrmQualityEvaluationStatuses.Ids.Completed).ToList();
        ...
        var agentStats = completed
            ...
            .Select(g => new CrmQualityAgentScoreDto
            {
                PersonnelId = g.Key,
        ...
        return new CrmQualityDashboardDto
        {
            TotalEvaluations = evaluations.Count,
            CompletedEvaluations = completed.Count,
            PendingEvaluations = evaluations.Count(e =>
                e.StatusId == CrmQualityEvaluationStatuses.Ids.Pending ||
                e.StatusId == CrmQualityEvaluationStatuses.Ids.Draft ||
                e.StatusId == CrmQualityEvaluationStatuses.Ids.InProgress),
    ...
    private async Task<CrmQualityChecklist?> GetChecklistByUidAndCustomer(Guid uid, int? customerId)
    {
    ...
    private static CrmQualityChecklistDto MapChecklistToDto(CrmQualityChecklist c) => new()
    {
        Id = c.Id,
        Uid = c.Uid,
        Name = c.Name,
        ...
        ScoringMethodName = CrmQualityScoringMethods.GetById(c.ScoringMethodId)?.Description ?? "",
    ...
    private static CrmQualityQuestionDto MapQuestionToDto(CrmQualityQuestion q) => new()
    {
        Id = q.Id,
        ...
        ScoringTypeName = CrmQualityScoringTypes.GetById(q.ScoringTypeId)?.Description ?? "",
        WeightPoints = q.WeightPoints,
        MaxPoints = q.MaxPoints,
        PenaltyTypeId = q.PenaltyTypeId,
        PenaltyTypeName = CrmQualityPenaltyTypes.GetById(q.PenaltyTypeId)?.Description ?? "",
        ...
        IsActive = q.IsActive,
        SubCriteria = q.SubCriteria?.Select(sc => new CrmQualitySubCriteriaDto
        {
            Id = sc.Id,
    ...
    private static CrmQualityEvaluationDto MapEvaluationToDto(CrmQualityEvaluation e) => new()
    {
        Id = e.Id,
        Uid = e.Uid,
        ...
        StatusId = e.StatusId,
        StatusName = CrmQualityEvaluationStatuses.GetById(e.StatusId)?.Description ?? "",
