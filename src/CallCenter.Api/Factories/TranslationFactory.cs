...
public class TranslationFactory : ITranslationFactory
{
    ...
    public TranslationFactory(ITranslationKeyEntityService translationEs, ITranslationService translationService, IUnitOfWork uow)
    {
        _translationEs = translationEs;
    ...
    public async Task<(bool Success, string? Message)> ImportXmlAsync(Stream xmlStream, string? userName)
    {
        ...
        foreach (var keyElement in keysElement.Elements("Key"))
        {
            ...
            var translationKey = await _translationEs.GetKeysQueryable()
            ...
            foreach (var valueElement in keyElement.Elements("Value"))
            {
                ...
                var translation = await _translationEs.GetTranslationsQueryable()
                ...
                {
                    _translationEs.AddTranslation(new Translation
                    {
    ...
    {
        if (await _translationEs.ExistsByKeyAsync(dto.Key))
    ...
    {
        var translationKey = await _translationEs.GetKeyByIdWithTranslationsAsync(id);
    ...
    public async Task<(bool Success, string? Error)> DeleteKeyAsync(int id)
    {
        ...
        _translationEs.RemoveTranslations(translationKey.Translations);
        _translationEs.RemoveKey(translationKey);
        await _uow.SaveChangesAsync();
