...
public class IvrFactory : IIvrFactory
{
    ...
    {
        return await _ivrEs.GetGreetingsQueryable()
    ...
    {
        var entity = await _ivrEs.GetGreetingByIdAsync(id, customerId);
    ...
    {
        return await _ivrEs.GetMenusQueryable()
    ...
    {
        var menu = await _ivrEs.GetMenusQueryable()
    ...
    {
        var menu = await _ivrEs.GetMenuByIdWithOptionsAsync(id, customerId);
        ...
        {
            _ivrEs.RemoveMenuOptions(menu.Options);
            foreach (var opt in request.Options)
            {
                menu.Options.Add(new IvrMenuOption
                {
    ...
    {
        var menu = await _ivrEs.GetMenuByIdAsync(id, customerId);
        if (menu == null) return (false, "IVR menusu bulunamadi");

        _ivrEs.RemoveMenu(menu);
        await _uow.SaveChangesAsync();
        return (true, null);
    }
    ...
    {
        return await _ivrEs.GetHoldMusicsQueryable()
    ...
    public async Task<HoldMusicDto> CreateHoldMusicAsync(int customerId, CreateHoldMusicRequest request, string audioFilePath, string? audioFileName)
    {
        ...
        {
            var others = await _ivrEs.GetHoldMusicsQueryable()
                .Where(h => h.CustomerId == customerId && h.IsDefault)
                .ToListAsync();
            foreach (var o in others) o.IsDefault = false;
        }

        _ivrEs.AddHoldMusic(entity);
        await _uow.SaveChangesAsync();
    ...
    {
        var entity = await _ivrEs.GetHoldMusicByIdAsync(id, customerId);
        if (entity == null) return (false, "Bekleme muzigi bulunamadi");

        _ivrEs.RemoveHoldMusic(entity);
        await _uow.SaveChangesAsync();
        return (true, null);
    }
    ...
    {
        var hours = await _ivrEs.GetBusinessHoursQueryable()
    ...
    {
        var existing = await _ivrEs.GetBusinessHoursQueryable().Where(b => b.CustomerId == customerId).ToListAsync();
        _ivrEs.RemoveBusinessHoursRange(existing);

        foreach (var day in request.Days)
        {
            _ivrEs.AddBusinessHours(new BusinessHours
            {
                CustomerId = customerId,
    ...
    {
        return await _ivrEs.GetHolidaysQueryable()
            .Where(h => h.CustomerId == customerId)
    ...
    {
        var entity = await _ivrEs.GetHolidayByIdAsync(id, customerId);
        if (entity == null) return (false, "Tatil bulunamadi");

        _ivrEs.RemoveHoliday(entity);
        await _uow.SaveChangesAsync();
        return (true, null);
    }
    ...
    public async Task<IncomingCallConfigDto> GetIncomingCallConfigAsync(int customerId, int? queueId = null)
    {
        ...
        // 1. Tatil kontrolu
        var holiday = await _ivrEs.GetHolidaysQueryable()
            .Include(h => h.GreetingMessage)
            .Where(h => h.CustomerId == customerId &&
        ...
        if (holiday != null)
        {
            ...
            {
                var holidayGreeting = await _ivrEs.GetGreetingsQueryable()
        ...
        var businessHour = await _ivrEs.GetBusinessHoursQueryable()
        ...
        {
            var ivrMenu = await _ivrEs.GetMenusQueryable()
                .Include(m => m.Options).ThenInclude(o => o.TargetQueue)
        ...
        var holdMusic = queueId.HasValue
            ? await _ivrEs.GetHoldMusicsQueryable().FirstOrDefaultAsync(h => h.CustomerId == customerId && h.QueueId == queueId && h.IsActive)
            : null;
        holdMusic ??= await _ivrEs.GetHoldMusicsQueryable().FirstOrDefaultAsync(h => h.CustomerId == customerId && h.IsDefault && h.IsActive);
