...
public class CrmContactFactory : ICrmContactFactory
{
    private readonly ICrmContactEntityService _contacts;
    private readonly IUnitOfWork _uow;
    private readonly ILogger<CrmContactFactory> _logger;

    public CrmContactFactory(ICrmContactEntityService contacts, IUnitOfWork uow, ILogger<CrmContactFactory> logger)
    {
        _contacts = contacts;
        _uow = uow;
        _logger = logger;
    ...
    {
        var result = new BulkImportResult { TotalRows = req.CrmContacts.Count };
        ...
        foreach (var item in req.CrmContacts)
        {
            if (!item.IsValid)
            {
                result.SkippedCount++;
