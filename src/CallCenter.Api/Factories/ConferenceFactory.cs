...
{
    private readonly IConferenceEntityService _conferenceEs;
    private readonly IJanusService _janus;
    private readonly IUnitOfWork _uow;
    private readonly ILogger<ConferenceFactory> _logger;

    public ConferenceFactory(IConferenceEntityService conferenceEs, IJanusService janus, IUnitOfWork uow, ILogger<ConferenceFactory> logger)
    {
        _conferenceEs = conferenceEs;
        _janus = janus;
        _uow = uow;
        _logger = logger;
    }
    ...
    {
        var room = await _conferenceEs.GetRoomsQueryable()
    ...
    {
        var query = _conferenceEs.GetRoomsQueryable()
            .Include(r => r.CreatedByUser)
            .Include(r => r.Participants).ThenInclude(p => p.User)
    ...
    {
        var room = await _conferenceEs.GetRoomByIdWithParticipantsAsync(roomId);
        ...
        _conferenceEs.AddParticipant(participant);
        await _uow.SaveChangesAsync();
    ...
    {
        var participant = await _conferenceEs.GetRoomsQueryable()
