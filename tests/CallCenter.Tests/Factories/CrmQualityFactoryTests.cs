...
public class CrmQualityFactoryTests : IDisposable
{
    private readonly AppDbContext _db;
    private readonly CrmQualityFactory _sut;

    public CrmQualityFactoryTests()
    {
        ...
        var checklistEs = new CrmQualityChecklistEntityService(_db);
        var questionEs = new CrmQualityQuestionEntityService(_db);
        var subCriteriaEs = new CrmQualitySubCriteriaEntityService(_db);
        var evaluationEs = new CrmQualityEvaluationEntityService(_db);
        var answerEs = new CrmQualityAnswerEntityService(_db);
        var selectionEs = new CrmQualityAnswerSubCriteriaSelectionEntityService(_db);
        var thresholdEs = new CrmQualityScoreThresholdEntityService(_db);
        var callRecordEs = new CallRecordEntityService(_db);
        var personnelEs = new CustomerPersonnelEntityService(_db);
        var uow = new UnitOfWork(_db);

        _sut = new CrmQualityFactory(
    ...
    public async Task CreateChecklist_ValidRequest_CreatesSuccessfully()
    {
        var req = new CreateChecklistRequest
        {
            ...
            ScoringMethodId = CrmQualityScoringMethods.Ids.Maximum,
            MaxTotalPoints = 100,
            IsScored = true
        };
        ...
        result.ScoringMethodId.Should().Be(CrmQualityScoringMethods.Ids.Maximum);
    ...
    public async Task AddQuestion_IncreasesQuestionCount()
    {
        ...
        var (success, error, question) = await _sut.AddQuestionAsync(cl.Uid, new CreateQuestionRequest
        {
            ...
            WeightPoints = 10,
            MaxPoints = 5,
            ScoringTypeId = CrmQualityScoringTypes.Ids.Scored
        }, CustomerId);
    ...
    public async Task CreateEvaluation_CreatesAsDraft()
    {
        ...
        success.Should().BeTrue();
        ev.Should().NotBeNull();
        ev!.StatusId.Should().Be(CrmQualityEvaluationStatuses.Ids.Draft);
    ...
    public async Task SaveDraft_SavesAnswersWithoutScoring()
    {
        ...
        detail!.StatusId.Should().Be(CrmQualityEvaluationStatuses.Ids.Draft);
    ...
    public async Task SubmitEvaluation_CalculatesScores_Maximum()
    {
        ...
        var detail = await _sut.GetEvaluationByUidAsync(ev.Uid, CustomerId);
        detail!.StatusId.Should().Be(CrmQualityEvaluationStatuses.Ids.Completed);
    ...
    public async Task SubmitEvaluation_MaximumMethod_CorrectCalculation()
    {
        ...
        var cl = await CreateTestChecklist("Form", CrmQualityScoringMethods.Ids.Maximum);
    ...
    public async Task SubmitEvaluation_PenaltyCards_Counted()
    {
        ...
        await _sut.AddQuestionAsync(cl.Uid, new CreateQuestionRequest
        {
            ...
            ScoringTypeId = CrmQualityScoringTypes.Ids.Penalty,
            PenaltyTypeId = CrmQualityPenaltyTypes.Ids.RedCard
        }, CustomerId);
        await _sut.AddQuestionAsync(cl.Uid, new CreateQuestionRequest
        {
            Text = "Yanlis bilgi", WeightPoints = 0, MaxPoints = 1,
            ScoringTypeId = CrmQualityScoringTypes.Ids.Penalty,
            PenaltyTypeId = CrmQualityPenaltyTypes.Ids.YellowCard
        }, CustomerId);
        ...
        await _sut.SubmitEvaluationAsync(ev!.Uid, new SubmitEvaluationRequest
        {
            Answers = new List<SubmitAnswerRequest>
            {
                new() { QuestionId = qRed.Id, IsPenaltyApplied = true, AppliedPenaltyTypeId = CrmQualityPenaltyTypes.Ids.RedCard },
                new() { QuestionId = qYellow.Id, IsPenaltyApplied = true, AppliedPenaltyTypeId = CrmQualityPenaltyTypes.Ids.YellowCard }
            }
        }, CustomerId);
    ...
    public async Task CancelEvaluation_ChangesStatus()
    {
        ...
        success.Should().BeTrue();
        var detail = await _sut.GetEvaluationByUidAsync(ev.Uid, CustomerId);
        detail!.StatusId.Should().Be(CrmQualityEvaluationStatuses.Ids.Cancelled);
    }
    ...
    private async Task<CrmQualityChecklistDto> CreateTestChecklist(string name = "Test Form", int scoringMethod = 1)
    {
    ...
    private async Task<CrmQualityChecklistDetailDto> CreateTestChecklistWithQuestions()
    {
    ...
    private async Task<(CrmQualityEvaluationDto Evaluation, CrmQualityChecklistDetailDto Checklist)> CreateTestEvaluation()
    {
        var cl = await CreateTestChecklistWithQuestions();
        var (_, _, ev) = await _sut.CreateEvaluationAsync(new CreateEvaluationRequest
        {
